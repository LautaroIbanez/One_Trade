...
def bootstrap_components(components_source, concurrency, install_type):
    ...
    if status or not out:
        ...
        status_print(
            f"[PASS] Finished installing npm dependencies for component packages: {source_glob} [PASS]",
            file=sys.stderr,
        )
    ...
    for package in source_glob.split(","):
        ...
        if not os.path.exists(build_directory):
            ...
            with open(os.path.join(dest_path, ".gitkeep"), "w", encoding="utf-8"):
                pass
            status_print(
                f"[PASS] Finished moving build artifacts from {build_directory} to Dash [PASS]"
            )
