...
class TestFallbackImprovements(unittest.TestCase):
    ...
    def test_fallback_skips_trade_when_no_future_candles(self):
        """Test that fallback skips trade when no future candles exist for exit simulation."""
        date = datetime(2024, 1, 3, tzinfo=timezone.utc)
        session_data = self._make_bullish_session(date)
        # Don't add next day data - this should cause fallback to skip the trade
        ...
        config = {
            ...
            'orb_window': (11, 12),
            'entry_window': (11, 13),
            'full_day_trading': False,  # Not full day trading
            'force_one_trade': True,
        }
        
        strategy = SimpleTradingStrategy(config)
        trades = strategy.process_day(session_data, date.date())
        
        # Should return empty trades when no future candles available
        self.assertEqual(len(trades), 0, "Should skip trade when no future candles for exit simulation")
